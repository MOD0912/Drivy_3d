...
'''
Small description of code:
vy==> velocity in y Axis
ay==> acceleration in y Axis


'''
MODEL_PATH = "bestestest.pt"
...
class RobotControlGUI:
    ...
    def __init__(self, root):
        ...
        self.status_label = tk.Label(root, textvariable=self.status, font=("Helvetica", 14, "bold"),bg="#2c3e50", fg="#ecf0f1")
        ...
        tk.Button(controller_frame, text="â†‘", command=self.drivy_up,
                  bg="#3498db", fg="white", activebackground="#2980b9", **button_config
                  ).grid(row=0, column=1, pady=(0, 5))
        tk.Button(controller_frame, text="â†", command=self.drivy_left,
                  bg="#3498db", fg="white", activebackground="#2980b9", **button_config
                  ).grid(row=1, column=0, padx=(0, 5))
        tk.Button(controller_frame, text="â†’", command=self.drivy_right,
                  bg="#3498db", fg="white", activebackground="#2980b9", **button_config
                  ).grid(row=1, column=2, padx=(5, 0))
        tk.Button(controller_frame, text="â†“", command=self.drivy_down,
                  bg="#3498db", fg="white", activebackground="#2980b9", **button_config
        ...
        # Start remote control server (for Melvicontrol)
    ...
    def run_video(self):
        try:
            ...
            if self.framecount > 3 and self.results:
                for result in self.results:
                    for box in result.boxes:
                        ...
                        label = f"{model.names[cls]}: {conf:.2f} Area: {area}"
            ...
            #Dont crash on camera hiccups; show status once
    ...
    def accept_loop(self):
        ...
        self.status.set("Waiting for ESP32â€¦")
    ...
    def read_discard_loop(self, conn):
        try:
            ...
            self.conn = None
            self.addr = None
            self.status.set("Disconnected. Waiting for ESP32â€¦")
    ...
    def start_remote_server(self):
        t = threading.Thread(target=self.remote_server_loop, daemon=True)
        t.start()

    def remote_server_loop(self):
        remote_port = 5002
        ...
        try:
            ...
            s.listen(1)
            print(f"Remote control listening on port {remote_port}")
            while True:
                try:
                    conn, addr = s.accept()
                    print(f"Remote control connected: {addr}")
                    with conn:
                        while True:
                            data = conn.recv(1024)
                            if not data: break
                            commands = data.decode(errors='ignore').strip().split('\n')
                            for cmd in commands:
                                cmd = cmd.strip().upper()
                                if cmd == "UP": self.root.after(0, self.drivy_up)
                                elif cmd == "DOWN": self.root.after(0, self.drivy_down)
                                elif cmd == "LEFT": self.root.after(0, self.drivy_left)
                                elif cmd == "RIGHT": self.root.after(0, self.drivy_right)
                                elif cmd == "STOP": self.root.after(0, self.drivy_stop)
                except Exception as e:
                    print(f"Remote connection error: {e}")
        except Exception as e:
            print(f"Could not start remote server: {e}")
